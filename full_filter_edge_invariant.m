%{
    Filter edge invariant mapping structure in one go.
%}


disp(strcat("Running '", mfilename, "' ..."));
load('config.mat');
load(strcat("data/PR_result/complete/", spec_restart, "_result.mat"));
...
[PR_solution_collection_unique, group_rep, group_idx] = unique(page_rank_score_solution_collection, 'rows');
group_mapping_by_PR = repelem({[]}, length(group_rep));

for i = 1:length(group_rep)
    group_mapping_by_PR {i} = find(group_idx==i); % as guidance
end
...
foldername = "data/distinct_structure/complete/";
filename = strcat(foldername, spec_restart, "_distinct.mat");
if ~exist(foldername, 'dir')
       mkdir(foldername)
end
save(filename, 'perm_solution_mapping', 'group_mapping_by_PR', 'group_rep', 'group_idx');