% run script to perform action for multiple configurations
...
bit_len = 3; % default 3 bit len
for num_phenotype = 2:8
    for j = 1:length(restart_prob_list)
        restart_prob = 0.55; % [0.05, 0.3, 0.55, 0.85]
        % restart_prob = restart_prob_list(j);
